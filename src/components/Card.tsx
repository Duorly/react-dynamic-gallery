...
export const Card: React.FC<Props> = ({ car, onLike, onDelete }) => {
    return (
        <article className="group relative break-inside-avoid overflow-hidden rounded-3xl bg-slate-900 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 hover:brightness-110">
            ...
            <div className="absolute bottom-0 w-full p-5 flex justify-between items-end">
                <div>
                    <h3 className="text-white font-semibold text-lg tracking-tight drop-shadow">{car.title}</h3>
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        ...
                        <span className="text-gray-200 font-medium">{car.likes}</span>
                    </button>

                    <button
                        onClick={() => onDelete(car.id)}
                        aria-label={`Delete ${car.title}`}
                        className="px-3 py-1.5 rounded-full border border-rose-500 text-rose-400 text-sm hover:bg-rose-600/10 transition"
                    >
                        Supprimer
                    </button>
                </div>
